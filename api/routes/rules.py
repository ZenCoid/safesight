...
async def create_rule(rule_def: RuleDefinition, db: AsyncSession = Depends(get_db)):
    db_rule = Rule(name=rule_def.rule_name, version=rule_def.version, definition=rule_def.dict())
    db.add(db_rule)
    await db.commit()
...
async def update_rule(rule_id: str, rule_def: RuleDefinition, db: AsyncSession = Depends(get_db)):
